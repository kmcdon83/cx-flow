# Troubleshooting CxFlow

## Common Problems

### SSL / TLS

If any of the connecting components are using an internal CA or
self-signed certificate, the Java runtime used must trust the
appropriate certificates.  This can be found under the JRE folder of the
installed Java, and under **lib/security**. The Java truststore is
typically named **cacerts **and has a default passphrase of
**changeit **- note, this is outside of the control of Checkmarx and
CxFlow.

### XML encoding

There have been cases where Checkmarx REST API responds with XML that is
not well encoded, and some defensive programming logic has been
introduced to remove non-UTF-8 compatible characters.  This issue
presents itself from time to time and detailed logging is required to
find the exact problem - See below.  

### Debugging

Debug logs can be enabled by adding the following configuration to the
yaml (*application.yml*, or *application-\<profile\>.yml*) properties or
by adding additional command line arguments when launching.
...
Events that drive scanning/results feedback are given a unique ID and
that ID is passed through the various logs to ensure you can make an
link between all of the events.
...
``` java
<cx-flow command> --logging.level.org.springframework.web.client.RestTemplate=TRACE --logging.level.com.checkmarx.flow.service=DEBUG --logging.level.org.apache.http.wire=TRACE 
```
...
If you run the Java 8 bundled version of Cx Flow but are using a system
JRE/JDK of 10+, you will have unsatisfied dependencies regarding
SAX/JAXB parsing objects.  This is due to the fact that they have been
removed in JRE 10 onward.  The message looks like the follow -  simply
ensure the correct JAR/JRE is being leveraged to avoid this.  The need
to maintain separate builds per JRE is linked to the use of SOAP based
APIs for Checkmarx, which only 3 APIs are still used (Login,
GetIssueDescription, CreateTeam).  The XML parsing is also required for
ingesting the XML results from Checkmarx.  If both move to a REST/JSON
based API request/response model, then the dependency will be removed.
